...
# Constants
SKIP_NODE_ASSIGNMENT = {"broker", "recorder", "grid", "forecasting", "heat"}
...
def create_heat_config(conf: DictConfig, output_dir: Path) -> None:
    """Generate heat_config.json for the heat federate."""
    heat_config = {
        "name": "heat",
        "loglevel": conf.general.loglevel,
        "coreType": "zmq",
    ...
    config_path = output_dir / "heat_config.json"
    with open(config_path, "w") as f:
        json.dump(heat_config, f, indent=4)
    print(f"Generated {config_path.name}")
...
def main():
    ...
    # Generate heat config file
    create_heat_config(conf, output_dir)
