...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_cpu", default=1, help="Number of CPUs available for parallel generation"
    )
    ...
    print(
        f"Running input file {inputfile} (ID {args.inputfile_id}) with {settings["n_cpu"]} CPUs!"
    )
    # ----------------------------------------------------------------------------------------#
    # ----------------------------------------------------------------------------------------#
    ...
    end_time = time.time()
    print(f"Running script took {(end_time-start_time)/60:.3f} minute(s)!")
