...
from src.external.emobpy.tools import (
    Unit,
    check_for_new_function_name,
    _add_column_datetime,
    display_all,
)
...
class Weather:
    ...
    def pressure(self, country_code, year):
        ...
        """
        return (
            self._load_data(country_code, year, option="pressure Pascal", location=None)
            / 100
        )
    ...
    def _load_data(country_code="DE", year=2016, option="temp Kelvin", location=None):
        ...
        if country_code not in country_timezones:
            raise Exception(
                f"Country code {country_code} not in file: {country_timezones_file}. Select another country code or include it in the file."
            )
            ...
            for model, model_values in brand_values.items():
                for year, year_values in model_values.items():
                    for param, param_value in year_values.items():
                        if (
                            param_value.__class__.__name__ in ["float", "int"]
            ...
            for model, model_values in brand_values.items():
                for year, year_values in model_values.items():
                    for param, param_value in year_values.items():
                        if (
                            param == parameter
                            and param_value.__class__.__name__ in ["float", "int"]
                            and param_value is not None
                        ):
...
class ModelSpecs:
    ...
    def add_calculated_param(self):
        ...
        """
        if "power" in self.parameters and "curb_weight" in self.parameters:
        ...
        if "curb_weight" in self.parameters and "axle_ratio" in self.parameters:
    ...
    def add_fallback_data(self):
        for parameter in VEHICLE_NEEDED_PARAMETERS:
            if parameter not in self.parameters:
                ...
                logger.info(
                    f"Fallback value {fallback} added for missing {parameter} parameter."
                )
...
class DrivingCycle:
    ...
    def _select_driving_cycle_index(
        ...
        if not full_driving_cycle:
            for k, v in self.index_speed[driving_cycle_type][WLTC_class][
                ...
                idx.append(k)
                driving_cycle_type_average_speed = (
                    Unit(v["value"], v["unit"]).convert_to("m/s").val
                )
                trip_average_speed = (
                    Unit(speed["value"], speed["unit"]).convert_to("m/s").val
                )
    ...
    def driving_cycle(self, trip, model, full_driving_cycle=False):
        ...
        if full_driving_cycle:
            temp = Unit(
                ...
                self.data[trip.index]["time"]["unit"],
            )
            temp.convert_to("s")
            ...
            temp = Unit(
                ...
                self.data[trip.index]["distance"]["unit"],
            )
            temp.convert_to("m")
        ...
        trip.time["value"] = np.ceil(
            Unit(trip._duration["value"], trip._duration["unit"]).convert_to("s").val
        )
        trip.time["unit"] = "s"
        ...
        speed_array = (
            normalized_array
            * Unit(trip._mean_speed["value"], trip._mean_speed["unit"])
            .convert_to("m/s")
            .val
        )
        i = 0
...
class Trip:
    ...
    def get_mean_speed(self):
        ...
        self._distance["value"] = (
            Unit(self.distance["value"], self.distance["unit"]).convert_to("m").val
        )
        self._distance["unit"] = "m"
        self._duration["value"] = (
            Unit(self.duration["value"], self.duration["unit"]).convert_to("s").val
        )
    ...
    def _check(self):
        ...
        for lyr in self.layer_names:
            if self.layer_conductivity[lyr] is None:
                logger.info(
                    f"{self.__class__.__name__}.layer_conductivity['{lyr}'] = None"
                )
                flag = True
        ...
        for lyr in self.layer_names:
            if self.layer_thickness[lyr] is None:
                logger.info(
                    f"{self.__class__.__name__}.layer_thickness['{lyr}'] = None"
                )
                flag = True
        if flag:
            self.flag = True
            logger.info(
                'There are still "None" fields. All fields must contain values.'
            )
        else:
            self.flag = False
...
class Consumption:
    ...
    def _ev_par_test(self):
        ...
        if len(param_missing) == 0:
            ...
            for name in param_missing:
                logger.info(name)
            raise Exception(
                "Parameters missing, add these parameters to the ev_model object < "
    ...
    def run(
        ...
        self.battery_discharge_eff = self.vehicle.parameters["battery_discharging_eff"]
        ...
        if heat_insulation.flag:
            raise Exception(
                "heat_insulation object is not complete. Test the object with the method check() before "
        ...
        dc = DrivingCycle()
        dc.load_data()
        len(self.profile[self.profile["state"] == "driving"])
        current = 1
        ...
        for i, row in self.profile.iterrows():
            if row["state"] == "driving":
                ...
                P_max = self.vehicle.parameters["power"] * 1000  # kW to W
    ...
    def _fill_rows(self):
        ...
        if self.freq == "1s":
            for trip in trip_data:
                ...
                self.timeseries.loc[
                    trip_start : trip_start + trip_length - 1,
                    "instant consumption in W",
                ] = trip_values
            self.timeseries.loc[:, "average power in W"] = self.timeseries.loc[
                :, "instant consumption in W"
            ]
        else:
            for trip in trip_data:
                ...
                if trip_length < step_secs:
                    ...
                    for i in range(0, trip_length, step_secs):
                        ...
                        if last_second > trip_length:
                            ...
                            trip_values = (
                                trip.results["P_bat_actual"][i:last_second].sum()
                                / step_secs
                            )
                            self.timeseries.loc[trip_section, "average power in W"] = (
                                trip_values
                            )
                        else:
                            trip_values = trip.results["P_bat_actual"][
        ...
        self.timeseries["consumption"] = (
            self.timeseries["consumption"] * self.timeseries["distance"]
        ).fillna(0) / (
            self.timeseries["distance"].replace(0, 1)
        )  # this is an artifact to make zero the consumption when distance is zero
