...
from src.external.emobpy.constants import TIME_FREQ
    ...
    def _testing_soc(self):
        ...
        if self.failed_chrg.empty:
            if self.dt["soc"].iloc[-1] >= self.soc_init:
                ...
                logger.info(
                    f"soc_init:{str(round(self.soc_init, 3))} --> soc_end:{str(self.soc_end)}"
                )
                self.success = True
                self.notation = "True"
                self.ready = True
                ...
                if self.dt["soc"].iloc[-1] >= self.soc_init:
                    ...
                    if self.stored_n == 3:
                        ...
                        self.ready = True
                        logger.debug(
                            f"Consumption set zero for the last trips. Time steps share:{str(self.proportion_ts_modified)}"
                        )
                        self.soc_end = round(self.dt["soc"].iloc[-1], 3)
                        logger.info(
                            f"soc_init:{str(round(self.soc_init, 3))} --> soc_end:{str(self.soc_end)}"
        ...
        if not self.ready:
            if self.n % 40 == 0:
                if self.n != 0:
                    logger.debug(
                        f"still in while loop after {str(self.n)} iterations. Battery may be small, or few charging points available..."
                    )
            if self.n % 80 == 0:
                if self.n != 0:
                    if self.battopt:
