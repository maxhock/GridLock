...
def rolling_resistance_coeff_M1(temp, v, road_type=0):
    ...
    return (1.9e-6 * temp**2 - 2.1e-4 * temp + 0.013 + 5.4e-5 * v) * factor[road_type]
...
@numba.jit(nopython=True)
def prollingresistance(rolling_resistance_coeff, vehicle_mass, g, v, slop_angle=0):
    """
    ...
    """
    return (
        rolling_resistance_coeff * vehicle_mass * g * np.cos(np.deg2rad(slop_angle)) * v
    )
...
def pairdrag(air_density, frontal_area, drag_coeff, v, wind_speed=0):
    ...
    Returns:
        float: [description]
    """
    return 1 / 2 * air_density * frontal_area * drag_coeff * (v - wind_speed) ** 2 * v
...
def qhvac(
    ...
    for tm in range(vehicle_speed.shape[0]):
        ...
        Q[tm][1] = Q_in_per
        Q_in_vent = q_ventilation(D(T_out, P_out, h=h_out), flow_air, cp(T_out), T_out)
        Q[tm][2] = Q_in_vent
        Q_out_vent = q_ventilation(
            D(t, P_out, h=h_out), mass_flow_in / D(t, P_out, h=h_out), cp(t), t
        )
        Q[tm][3] = Q_out_vent
        Q_tr = q_transfer(
            zone_layer,
        ...
        Q[tm][4] = Q_tr
        Q[tm][0] = (
            cabin_volume * D(t, P_out, h=h_out) * cp(t) * (t - t_1)
            - Q_in_per
            - Q_in_vent
            + Q_out_vent
            + Q_tr
...
def plot_multi(data, cols=None, spacing=0.1, **kwargs):
    """#TODO DOCSTRING
...
def balance(db, tscode, include=None):
    ...
    if db.db[tscode]["kind"] != "consumption":
        raise Exception(
            "code '{}' does not correspond to a consumption profile".format(tscode)
        )
    if include is None:
        # all trips
        flag = True
        ...
        consumption = db.db[tscode]["Trips"].trips[include].consumption["value"]
