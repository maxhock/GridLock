...
class Unit:
    ...
    def convert_to(self, new_unit, msg=False):
        ...
        def change_unit(self, new_unit, msg):
            ...
            if msg:
                logger.info(
                    f"Unit of value changed: {self.val} {self.unit} -> {self.val * scale[new_unit]} {new_unit}"
                )
    ...
    def info(self):
        logger.info(
            f"val: {self.val}, unit: {self.unit}, description: {self.description}"
        )
...
def set_seed(seed=None, dir="config_files"):
    ...
    if seed is None:
        try:
            ...
            logger.info(
                "As seed.txt does not contain a number, a random value has been written. This number is useful when you want to replicate the same results in another OS or PC."
            )
            seed = int(time.time())
        ...
        if os.path.isfile(os.path.join(dir, "seed.txt")):
            ...
            logger.debug(
                f"seed.txt has been deleted to replace it with the given seed {seed}."
            )
        with open(os.path.join(dir, "seed.txt"), "w") as f:
            f.write(str(seed))
...
def check_msg_arg_conditions(msg_id):
    ...
    try:
        ...
        if status["enabled"]:
            ...
            formatted_expiry_date = datetime.datetime.strptime(
                MSG_CONF[msg_id]["expiry_date"], "%Y-%m-%d"
            ).date()
