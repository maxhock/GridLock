...
class Charging:
    ...
    def load_scenario(self, database):
        ...
        if database.db[self.input]:
            if database.db[self.input]["kind"] == "availability":
                ...
                self.capacity_charging_point = database.db[self.input]["chargingdata"][
                ...
                raise ValueError(
                    "The charging availability profile {} can not be found in the database".format(
                        self.input
                    )
                )
        else:
    ...
    def run(self):
        ...
        if self.option == "immediate":
            ...
            try:
                ...
                self.success = False
                logger.info(
                    '"{}" is not in "{}". Availability profile: {}'.format(
                        self.point, " - ".join(self.states), self.input
                    )
                )
                self._clean()
                return None
            ...
            self.arraycodestate = np.array(
                [self.states.index(s) for s in self.arraystringstate]
            )
            self.numpy_array2 = self.profile[["consumption", "charging_cap"]].values.T
            self.results = self._immediate(
                self.pointcode,
                self.charging_eff,
            ...
            try:
                ...
                self._clean()
                return None
            self.numpy_array3 = self.profile[
                ["state", "consumption", "charging_cap"]
            ].values.T
            self.arraystringstate = self.numpy_array3[0]
            self.arraycodestate = np.array(
                [self.states.index(s) for s in self.arraystringstate]
            ...
            self.profile.loc[:, "charge_battery"] = self.results[1]
            self.profile.loc[:, "charge_grid"] = self.results[2]
        elif set(["from", "to", "at"]).issubset(
            self.option.split("_")
        ):  # eg. 'from_22_to_6_at_home'
            self.op_list = [
                int(s) if represents_int(s) else s for s in self.option.split("_")
            ]
            self.from_ = self.op_list[1]
            self.to_ = self.op_list[3]
            self.point = self.op_list[5]

            self.numpy_array4 = self.profile[
                ["state", "consumption", "charging_cap", "hh"]
            ].values.T
            ...
            try:
                ...
                self.success = False
                logger.info(
                    'Charging point "{}" is not in "{}". Availability profile: {}'.format(
                        self.point, " - ".join(self.states), self.input
                    )
                )
                self._clean()
        ...
        for i in range(rows):
            if i == 0:
                ...
                if current_soc > 1:
                    soc[i] = 1
                    battery[i] = (
                        charging_cap[i] * t * charging_eff
                        - (current_soc - 1) * battery_capacity
                    ) / t
                    grid[i] = (
                        charging_cap[i] * t
                        - (current_soc - 1) * battery_capacity / charging_eff
                    ) / t
                else:
                    soc[i] = current_soc
                ...
                if state[i] == driving_code:
                    if zero == 1:
                        ...
                        if current_soc > 1:
                            ...
                            grid[i] = (
                                charging_cap[i] * t
                            ) / t  # I did not want to cancel t, just for code consistency
                else:
                    current_soc = (
                        zero
                        - consumption[i] / battery_capacity
            ...
            if state[i] == driving_code:
                ...
                if charging_cap[i] * t > 0:
                    ...
                    while state[j] == state[i]:
                        current_soc = (
                            cero
                            - consumption[j] / battery_capacity
                            + charging_cap[j] * t * charging_eff / battery_capacity
                        )
                        if current_soc > 1:
                            soc_ahead[j] = 1
                        else:
                    ...
                    k = (
                        j - i
                    )  # length in the same state, this will help to jump the queue
                    if np.any(soc_ahead[i:j] == 1):
                        ...
                        for m in range(i, j):
                            current_soc = (
                                cero
                                - consumption[m] / battery_capacity
                                + delta_charging * charging_eff / battery_capacity
                            )
                            if current_soc > 1:
                                soc[m] = 1
                                battery[m] = (
                                    delta_charging * charging_eff
                                    - (current_soc - 1) * battery_capacity
                                ) / t
                                grid[m] = (
                                    delta_charging
                        ...
                        for m in range(i, j):
                            current_soc = (
                                cero
                                - consumption[m] / battery_capacity
                                + charging_cap[m] * t * charging_eff / battery_capacity
                            )
                            if current_soc > 1:
                                soc[m] = 1
                                battery[m] = (
                                    charging_cap[m] * t * charging_eff
                                    - (current_soc - 1) * battery_capacity
                                ) / t
                                grid[m] = (
                                    charging_cap[m] * t
    ...
    def _A2BatPoint(
        ...
        while i < rows:  # code 1
            ...
            if (
                (state[i] == point)
                | (preferenceignore[i] == 1)
                | (state[i] == driving_code)
                | (point == -1)
            ):
                if (
                    (is_between(np.mod(hr[i], 24), (from_, to_)))
                    | (preferenceignore[i] == 1)
                    | (state[i] == driving_code)
                ):
                    if state[i] == driving_code:
                        ...
                        if zero < 0:
                            ...
                            while gap < 1.5:  # 1.5 kWh
                                ...
                                if state[n] == driving_code:
                                    gap = (
                                        gap
                                        - consumption[n]
                                        + charging_cap[n] * t * charging_eff * 0.5
                                    )
                                else:
                                    gap = (
                                        gap
                                        - consumption[n]
                                        + charging_cap[n] * t * charging_eff
                                    )
                                if n < 1:
                                    print(
                            ...
                            if np.any(soc_ahead[i:j] == 1):
                                soc_diff = 1 - zero
                                delta_charging = (
                                    soc_diff * battery_capacity / charging_eff / k
                                )
                                cero = zero
                                for m in range(i, j):
                                    current_soc = (
    ...
    def _check_success(self):
        ...
        if self.profile["actual_soc"].min() >= self.soc_min:
            ...
            sto_e = round(
                self.battery_capacity * self.profile["actual_soc"].values[-1], 2
            )
            bal_t = round(sto_i + chrg_t - cons_t - sto_e, 2)
            if bal_t != 0.0:
                logger.info(
                    f"Balance: {str(bal_t)}. Consumption: {str(cons_t)}. charge_battery: {str(chrg_t)}. sto_init: {str(sto_i)}. sto_end: {str(sto_e)}"
                )
            self.success = True
        else:
            self.success = False
