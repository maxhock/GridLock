...
class Users:
    ...
    def generate_annual_el_consumption(self):
        ...
        if self.building in {"SFH", "TH", "MFH", "AB"}:
            ...
            consumption_range = {
                "SFH": {
                    1: [1100, 1400, 1800, 2200, 2600, 3400, 4500, 4800],
                ...
                "MFH": {
                    1: [600, 800, 1000, 1300, 1500, 1700, 2100, 2300],
            ...
            for j in range(self.nb_flats):
                if self.building == "SFH":
                    random_nb = (
                        rd.random()
                    )  # picking random number in [0,1) to decide between which 2 values of consumption_range the annual electricity consumption lies
                    i = 1
                    while i <= 7:
                        if random_nb < sum(probabilities[:i]):
                            self.annual_el_demand_per_flat[j] = rd.randint(
                                consumption_range["SFH"][self.nb_occ[j]][i - 1],
                                consumption_range["SFH"][self.nb_occ[j]][i],
                            )
                ...
                if self.building == "MFH":
                    ...
                    while i <= 7:
                        if random_nb < sum(probabilities[:i]):
                            self.annual_el_demand_per_flat[j] = rd.randint(
                                consumption_range["MFH"][self.nb_occ[j]][i - 1],
                                consumption_range["MFH"][self.nb_occ[j]][i],
                            )
                            # A random integer is selected as the current demand, which must lie between the two values determined by the first random number
                            break
                        i += 1
    ...
    def calcProfiles(
        ...
        if self.building in {"SFH", "TH", "MFH", "AB"}:
            ...
            if building["buildingFeatures"]["NWG"] == 0:
                for j in range(self.nb_flats):
                    temp_obj = Profiles(
                        number_occupants=self.nb_occ[j],
                    ...
                    self.dhw = self.dhw + temp_obj.generate_dhw_profile(
                        building=building, holidays=holidays
                    )
                    self.annual_dhw_demand += self.dhw

                    # Occupancy profile in a flat
                    self.occ = (
                        self.occ + temp_obj.generate_occupancy_profiles_residential()
                    )
            ...
            self.gains = self.generate_gain_profile_elec_given(
                (elecDemand[building["buildingFeatures"]["bus"]] * 1000)
                .to_numpy()
                .flatten()
            )
    ...
    def calcHeatingProfile(self, site, envelope, holidays, time_resolution):
        ...
        (Q_H, Q_C, T_op, T_m, T_i, T_s) = heating.calc(
            envelope, site["T_e"], holidays, dt, self.building
        )
