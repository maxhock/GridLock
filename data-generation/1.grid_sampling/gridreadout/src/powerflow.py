...
def generate_random_loads(consumers):
    ...
    columns = pd.MultiIndex.from_product(
        [bus_values, ["p_mw", "q_mvar"]], names=["bus", "power_type"]
    )
...
def run_full_pf(net, new_loads):
    ext_grid_bus = int(
        net.ext_grid.loc[0, "bus"]
    )  # Which bus is the external import bus
    ...
    for i, row in new_loads.iterrows():
        ...
        if line_loadings.empty:
            ...
            line_loadings = pd.concat(
                [line_loadings, cur_line_load], axis=0, ignore_index=True
            )

        # Concat ext import
        cur_ext_import = pd.DataFrame(
            net_res.res_bus.loc[ext_grid_bus, ["p_mw", "q_mvar"]]
        ).T
        if ext_imports.empty:
            ext_imports = cur_ext_import
            ext_imports.index.name = "t"
        else:
            ext_imports = pd.concat(
                [ext_imports, cur_ext_import], axis=0, ignore_index=True
            )
