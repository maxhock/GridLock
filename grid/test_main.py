...
def test_create_federate(mocker):
    ...
    mocker.patch.object(grid_main.h, "helicsCreateValueFederateFromConfig", return_value=mock_fed)
    mocker.patch.object(grid_main.h, "helicsFederateRegisterSubscription", return_value=mock_sub)
    mocker.patch.object(grid_main.h, "helicsFederateRegisterGlobalPublication", return_value=mock_pub)
    mocker.patch.object(grid_main.h, "helicsInputGetName", return_value="mock_sub_key")
    mocker.patch.object(grid_main.h, "helicsPublicationGetName", return_value="mock_pub_key")
...
def test_run_federate_executes_loop(mocker):
    ...
    mocker.patch.object(grid_main.h, 'helicsFederateEnterExecutingMode', return_value=None)
    times = [0, 3600, 7200, 82800, 86400]
    mocker.patch.object(grid_main.h, 'helicsFederateRequestTime', side_effect=lambda fed, t: times.pop(0) if times else 86400)
    mocker.patch.object(grid_main.h, 'helicsInputIsUpdated', side_effect=lambda sub: False)
    mocker.patch.object(grid_main.h, 'helicsInputGetDouble', return_value=0.0)
    mocker.patch.object(grid_main.pp, 'runpp', side_effect=lambda net, numba=False: None)
    mocker.patch.object(grid_main.h, 'helicsPublicationPublishDouble', side_effect=lambda pub, val: None)
...
def test_main_runs(mocker):
    ...
    mocker.patch.object(grid_main, 'create_federate', return_value=(mocker.MagicMock(), mocker.MagicMock(), [], []))
