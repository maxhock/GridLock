...
def create_sample_data(n_days: int = 7):
    ...
    heeten_load_w = (
        np.resize(heeten_df["load"].to_numpy(dtype=float), n_steps)
        * WATTS_PER_KWH_PER_TIMESTEP
    )
    heeten_pv_w = (
        np.resize(heeten_df["pv"].to_numpy(dtype=float), n_steps)
        * WATTS_PER_KWH_PER_TIMESTEP
    )
    germany_price_eur_per_kwh = (
        np.resize(price_df["germany"].to_numpy(dtype=float), n_steps)
        * EUROS_PER_MWH_TO_EUROS_PER_KWH
    )
    ...
    df = pd.DataFrame(
        {
            "timestamp": pd.date_range(
                start="2024-01-01", periods=n_steps, freq="15min"
            ),
