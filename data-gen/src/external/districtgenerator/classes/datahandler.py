...
class Datahandler:
    ...
    def generateEnvironment(self, weatherData, site_data):
        ...
        self.site["location"] = [
            filtered_data.iloc[0]["Latitude"],
            filtered_data.iloc[0]["Longitude"],
        ]
        self.site["T_ne"] = filtered_data.iloc[0][
            "T_ne"
        ]  # norm outside temperature for calculating the design heat load
        self.site["T_me"] = filtered_data.iloc[0][
            "T_me"
        ]  # mean annual temperature for calculating the design heat load
        ...
        self.site["SunRad"] = sun.getSolarGains(
            initialTime=0,
        ...
        for id in self.scenario["id"]:
            try:
                ...
                if name in name_pool:
                    raise ValueError(
                        f"Building name '{name}' is not unique. ID collision detected."
                    )
                ...
                print(f"Error: {e}")
                print(
                    "The building ID must be a unique number to ensure proper identification and data tracking."
                )
                print(
                    f"Building with ID {id} will be skipped and not added to the district"
                )
                continue
    ...
    def generateBuildings(self):
        ...
        for building in self.district:
            with warnings.catch_warnings():
                ...
                if building["buildingFeatures"]["building"] not in [
                    "SFH",
                    "MFH",
                    "TH",
                    "AB",
                ]:
                    building["buildingFeatures"]["building"] = "AB"
                if (
                    building["buildingFeatures"]["building"] == "AB"
                    and building["buildingFeatures"]["year"] > 1978
                ):
                    building["buildingFeatures"]["year"] = 1978
            ...
            if building["buildingFeatures"]["year"] < 1860:
                building["buildingFeatures"]["year"] = 1860
            if math.isnan(building["buildingFeatures"]["year"]):
                building["buildingFeatures"]["year"] = 1970
            ...
            building["envelope"].heatload = building["envelope"].calcHeatLoad(
            ...
            # print(bldgs)
            # print(building)
            if isinstance(building["buildingFeatures"], pd.Series):
                building["buildingFeatures"] = building["buildingFeatures"].to_dict()
            index = bldgs["buildings_short"].index(
                building["buildingFeatures"]["building"]
            )
            building["buildingFeatures"]["mean_drawoff_dhw"] = bldgs[
    ...
    def generateDemands(self, elecDemand):
        ...
        for building in self.district:
            ...
            building["envelope"].calcNormativeProperties(
                self.site["SunRad"], building["user"].gains
            )
            ...
            space_heat_profiles[building["buildingFeatures"]["bus"]] = building[
                "user"
            ].heat
