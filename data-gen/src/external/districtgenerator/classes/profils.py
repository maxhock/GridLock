...
class Profiles:
    ...
    def generate_activity_profile_residential(self):
        ...
        if self.building in {"SFH", "TH", "MFH", "AB"}:
            activity = occ_residential.Occupancy(
                self.number_occupants, self.initial_day, self.nb_days
            )
    ...
    def generate_dhw_profile(self, building, holidays):
        ...
        for day in range(365):
            temperatur_mixed_water.append(
                45 + (3 * np.cos(math.pi * (2 / 365 * (day) - 2 * 355 / 365)))
            )
            ...
            temperatur_cold_water.append(
                10 + (7 * np.cos(math.pi * (2 / 365 * (day) - 2 * 225 / 365)))
            )
        ...
        temperature_difference_day = [
            T_out - T_in
            for T_out, T_in in zip(temperatur_mixed_water, temperatur_cold_water)
        ]

        temperature_difference = [
            T for T in temperature_difference_day for _ in range(24)
        ]
        self.temperature_difference = chres.changeResolution(
            temperature_difference, 3600, self.time_resolution, "mean"
        )
        ...
        dhw_profile = OpenDHW.generate_dhw_profile(
            ...
            occupancy=(
                ...
                if self.building in {"SFH", "TH", "MFH", "AB"}
                else self.number_occupants_building
            ),
            building_type=self.building,
            weekend_weekday_factor=(
                1.2 if self.building in {"SFH", "TH", "MFH", "AB"} else 1
            ),
            holidays=holidays,
        ...
        dhw_timeseries = OpenDHW.resample_water_series(
            dhw_profile, self.time_resolution
        )
        dhw_heat = OpenDHW.compute_heat(
            timeseries_df=dhw_timeseries, temp_dT=self.temperature_difference
        )

        return dhw_heat["Heat_W"].values

    def generate_el_profile_residential(
        self, holidays, irradiance, el_wrapper, annual_demand, do_normalization=True
    ):
        """
