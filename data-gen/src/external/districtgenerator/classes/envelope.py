...
class Envelope:
    ...
    def loadComponentProperties(self, prj):
        ...
        for name, elem in element_bind.items():
            if "OuterWall" in name:
                if (
                    elem["building_age_group"][0]
                    <= self.construction_year
                    <= elem["building_age_group"][1]
                    and elem["construction_type"]
                    == self.construction_type + "_1_" + self.usage_short
                ):
                    for lay in elem["layer"].items():
                        self.d["opaque"][comp] = np.append(
                            self.d["opaque"][comp], lay[1]["thickness"]
                        )
                        material_prop = self.loadMaterialID(
                            lay[1]["material"]["material_id"], material_bind
                        )
                        self.rho["opaque"][comp] = np.append(
                            self.rho["opaque"][comp], material_prop[1]
                        )
                        self.Lambda["opaque"][comp] = np.append(
                            self.Lambda["opaque"][comp], material_prop[2]
                        )
                        self.cp["opaque"][comp] = np.append(
                            self.cp["opaque"][comp], material_prop[3] * 1000
                        )
        ...
        for name, elem in element_bind.items():
            if "InnerWall" in name:
                dummy = min(2015, self.construction_year)  # data available until 2015
                if (
                    elem["building_age_group"][0]
                    <= dummy
                    <= elem["building_age_group"][1]
                    and elem["construction_type"] == "tabula_standard"
                ):
                    for lay in elem["layer"].items():
                        self.d["opaque"][comp] = np.append(
                            self.d["opaque"][comp], lay[1]["thickness"]
        ...
        for name, elem in element_bind.items():
            if "Window" in name:
                if (
                    ...
                    for lay in elem["layer"].items():
                        self.d["window"] = np.append(
                            self.d["window"], lay[1]["thickness"]
                        )
                        material_prop = self.loadMaterialID(
                            lay[1]["material"]["material_id"], material_bind
                        )
                        self.rho["window"] = np.append(
                            self.rho["window"], material_prop[1]
                        )
                        self.Lambda["window"] = np.append(
                            self.Lambda["window"], material_prop[2]
                        )
                        self.cp["window"] = np.append(
                            self.cp["window"], material_prop[3] * 1000
                        )
        ...
        for x in self.opaque_ext:
            ...
            self.U["opaque"][x] = 1.0 / (
                self.R_si["opaque"][x]
                + sum(self.d["opaque"][x] / self.Lambda["opaque"][x])
                + self.R_se["opaque"][x]
            )
        ...
        self.U["window"] = min(
            ...
            (
                ...
                / (
                    self.R_si["window"]
                    + sum(self.d["window"] / self.Lambda["window"])
                    + self.R_se["window"]
                )
            ),
        )
    ...
    def loadAreas(self, prj):
        ...
        try:
            ...
            self.A["opaque"]["roof"] = 1.2 * prj.buildings[self.id].outer_area[-2]
        ...
        try:
            ...
            self.A["window"]["east"] = prj.buildings[self.id].window_area[270.0]
        except KeyError:
    ...
    def calcHeatLoad(self, site, method="design"):
        ...
        f_g1 = (
            1.45  # Correction factor for annual fluctuation of the outdoor temperature
        )
        # Reduction factor
        ...
        if method == "design":
            Q_nHC = (
                self.A["opaque"]["wall"] * (self.U["opaque"]["wall"] + U_TB)
    ...
    def calculateHeatCapacity(self):
        self.C_m = sum(
            (self.kappa["opaque"][x] * self.A["opaque"][x]) for x in self.opaque
        )
    ...
    def calcNormativeProperties(self, SunRad, internal_gains):
        ...
        self.H_ve = self.rho_air * self.c_p_air * self.ventilationRate * self.V / 3600
        ...
        for t in range(len(SunRad[0])):
            for drct3 in range(len(directions)):  # for all directions
                if SunRad[drct3, t] > limit_shut_blinds:
                    self.I_sol["window", directions[drct3]][t] = (
                        0.15 * SunRad[drct3, t].copy()
                    )
        ...
        for t in range(len(SunRad[0])):
            ...
            for drct3 in direction3:
                A_j_k[t, drct3] = (
                    ...
                    * self.A["opaque"][drct3]
                    * (
                        self.alpha_Sc["opaque"]["wall"] * self.I_sol[drct3][t]
                        - self.h_r["opaque", "wall"]
                        * self.F_r[drct3]
                        * self.Delta_theta_er
                    )
                )
            ...
            for drct4 in direction4:
                A_j_k[t, drct4] = (
                    ...
                    * self.A["opaque"][drct4]
                    * (
                        self.alpha_Sc["opaque"][drct4] * self.I_sol[drct4][t]
                        - self.h_r["opaque", drct4]
                        * self.F_r[drct4]
                        * self.Delta_theta_er
                    )
                )
        ...
        for t in range(len(SunRad[0])):
            ...
            phi_sol[t] = (
                sum(A_j_k[t, drct3] for drct3 in direction3)
            ...
            self.phi_st[t] = (
                1 - self.A_m / self.A_tot - self.H_tr_w / 9.1 / self.A_tot
            ) * (0.5 * phi_int[t] + phi_sol[t])

            # thermal transmittance coefficient H_tr_em [W/K]
            # Simplification: H_tr_em = H_tr_op
            # (DIN EN ISO 13790, section 8.3, page 43)
            self.H_tr_em[t] = sum(
                self.A["opaque"][drct2] * self.U["opaque"][drct2] * self.b_tr[drct2][t]
