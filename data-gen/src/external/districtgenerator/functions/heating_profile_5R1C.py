...
def calc(zoneParameters, T_e, holidays, dt, building_type):
    ...
    for t in range(numberTimesteps):
        ...
        (t_op, t_m, t_i, t_s) = _calculateNoHeat(
            zoneParameters, T_e, t_m_previous, dt, timestep=t
        )

        if building_type in {"SFH", "TH", "MFH", "AB"}:
            if t_op < T_set and day not in range(135, 259):
                # Compute heat demand
                (q_hc, t_op, t_m, t_i, t_s) = _calculateHeat(
                    zoneParameters, T_e, T_set, t_m_previous, dt, timestep=t
                )
            elif t_op > T_set_ub:
                # Compute cooling demand
                (q_hc, t_op, t_m, t_i, t_s) = _calculateHeat(
                    zoneParameters, T_e, T_set_ub, t_m_previous, dt, timestep=t
                )
            else:
                # Nothing to do
                q_hc = 0
            ...
            if (
                t_op < T_set
                and day not in range(135, 259)
                and (day % 7 not in (0, 6) and day not in holidays)
            ):
                # Compute heat demand
                (q_hc, t_op, t_m, t_i, t_s) = _calculateHeat(
                    zoneParameters, T_e, T_set, t_m_previous, dt, timestep=t
                )
            elif t_op > T_set_ub and (day % 7 not in (0, 6) and day not in holidays):
                # Compute cooling demand
                (q_hc, t_op, t_m, t_i, t_s) = _calculateHeat(
                    zoneParameters, T_e, T_set_ub, t_m_previous, dt, timestep=t
                )
