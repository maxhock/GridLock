...
import numpy as np
import pandas as pd
from src.external.emobpy.constants import OPERATORS, WEEKS, RULE
...
class Mobility:
    ...
    def set_rules(self, rule_key=None, rules_path="rules.yml"):
        ...
        if self.stats_flag:
            if rule_key is None:
                self.user_rules = {}
                logger.info(
                    "set_rules: rules not provided, using default rules instead"
                )
            else:
    ...
    def run(self):
        ...
        self.prev_dest = (
            self.initial_state
        )  # it is the first state at the beginning of the profile
        ...
        if self.flag:
            ...
            if self.warningB % self.ratesteps == 0:
                ...
                if self.warningB == 0:
                    ...
                    self.logdf = pd.concat(
                        [self.logdf, pd.DataFrame.from_records([self.rate])],
                        sort=False,
                        ignore_index=True,
                    )
            ...
            if (
                ...
                if not self.flagiter:
                    self.flagiter = True
                logger.debug(
                    f"    Day {str(self.days)} 'select_tour' method in loop Nr. {self.warningB}. See log file {self.name}"
                )
