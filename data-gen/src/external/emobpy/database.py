...
class DataBase(object):
    ...
    def loadfiles_batch(
        self, loaddir="", batch=10, nr_workers=4, kind="", add_variables=[]
    ):
        """
        ...
        for lt in paths_batch:
            dc = {
                k: {"f": os.path.join(self.repo, v)}
                for k, v in enumerate(lt)
                if v.split(".")[-1] == "pickle"
            }
            odc = parallelize(
                self.loadpkl, dc, nr_workers, **dict(variables=variables, kind=kind)
            )
            for j in odc:
                if odc[j][1]:
...
class DataManager:
    ...
    def savedb(self, obj, dbdir="db_files"):
        ...
        if not obj.name:
            nnn = (
                "db_" + time.strftime("%Y%m%d_%H%M%S") + "_" + uuid.uuid4().hex[:5]
            )  # + time.strftime("%Y%m%d_%H%M%S")
            obj.name = nnn[:]
        os.makedirs(dbdir, exist_ok=True)
        with gzip.open(os.path.join(dbdir, obj.name + ".pickle"), "wb") as file:
