from config_grid import config
...
class DataBase:
    ...
    def read_trafo_pos(self, grid_specs):
        ...
        transformer = Transformer.from_crs(
            config.PYLOVO_COORD_FORMAT, config.TARGET_COORD_FORMAT, always_xy=True
        )
    ...
    def read_buildings(
        self, grid_specs, df_bus
    ):  # r.occupants, r.constructi, r.refurb_wal, r.refurb_roo, r.refurb_bas, r.refurb_win
        # query = text("""
        ...
        df_id["vertice_id"] = (
            df_bus["name"]
            .str.extract(r"^Consumer Nodebus (\d+)$")[0]
            .dropna()
            .astype(int)
        )
        ...
        def _get_loc(loc_string):
            ...
            match = re.match(
                r"POINT\(([-+]?[0-9]*\.?[0-9]+)\s*([-+]?[0-9]*\.?[0-9]+)\)", loc_string
            )
            if match:
                x = float(match.group(1))
                y = float(match.group(2))
                ...
                # Define the projections
                transformer = Transformer.from_crs(
                    config.PYLOVO_COORD_FORMAT,
                    config.TARGET_COORD_FORMAT,
                    always_xy=True,
                )
                # Convert from EPSG:3857 to EPSG:4326
                lon, lat = transformer.transform(x, y)
        ...
        df_buildings[["lat", "lon"]] = (
            df_buildings["center"].apply(_get_loc).apply(pd.Series)
        )
