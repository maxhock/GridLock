...
def _sample_sequence_with_tolerance(n, s, p, allowed_x, tol=0.99):
    ...
    for i in range(n - 1):
        ...
        for x in allowed_x:
            ...
            f_val = _compute_f(
                remaining_vars - 1, remaining_sum - x, p, allowed_x, cache, tol
            )
            weight = p[x] * f_val
            if weight > 0:
...
def _get_occupancy_distribution(prob: dict, n_hh: int, n_occ: int) -> list:
    ...
    if pd.isna(n_hh) or pd.isna(n_occ):
        ...
        if (
            max_dist_member * n_hh < n_occ
        ):  # statistics do not allow for filling up building, simply assing max occupants to each household
            return [max_dist_member] * n_hh
        elif (
            min_dist_member * n_hh > n_occ
        ):  # fewer occupants in building than covered by stats, simply assign min occupants to each household
            return [min_dist_member] * n_hh
        else:
            allowed_x = prob.keys()
            try:
                return _sample_sequence_with_tolerance(
                    n_hh, n_occ, prob, allowed_x, tol=0.95
                )
            except:
...
def _assign_household_occupancy(df_buildings):
    if len(df_buildings) == 0:
        ...
        prob = dict(
            zip(df_prob["size"], df_prob["probability"])
        )  # retrieve polynomial encoding household size probabilities
        df_buildings["occ_list"] = df_buildings.apply(
            lambda row: _get_occupancy_distribution(
                prob, row["houses_per_building"], row["occupants"]
            ),
            axis=1,
        )
        return df_buildings
...
def _get_total_demands(cdf, occ_list):
    if len(occ_list) == 0:
        ...
        for i, n_occ in enumerate(occ_list):
            ...
            demand_list.append(
                np.interp(
                    u[i], cdf[columns[n_occ - 1], "Y"], cdf[columns[n_occ - 1], "X"]
                )
            )
        return demand_list
...
def _assign_total_elec_demands(df_buildings):
    if len(df_buildings) == 0:
        ...
        df_buildings["demand_tot_list"] = df_buildings.apply(
            lambda row: _get_total_demands(df_cdfs, row["occ_list"]), axis=1
        )
        return df_buildings
...
def _assign_use_type(df_buildings):
    ...
    df_buildings["type"] = df_buildings.apply(
        lambda row: _get_use_type(df_type_dist, row["type"], row["use"]), axis=1
    )
    return df_buildings


def _get_single_building_elec_timeseries_res(
    yearly_demand_list, df_normalized_lps, lps_total_demands
):
    ts_list = []
...
def get_elec_demand(df_buildings):
    df_normalized_lps_res = pd.read_hdf(
        config.ELEC_LPS_PATH, key="df_normalized_scaled"
    )
    ...
    data_dict_res = {
        row["bus"]: _get_single_building_elec_timeseries_res(
            row["demand_tot_list"], df_normalized_lps_res, lps_res_total_demand
        )
        for idx, row in df_buildings.iterrows()
        if row["use"] == "Residential"
    }
    data_dict_ghd = {
        row["bus"]: _get_single_building_elec_timeseries_ghd(
    ...
    df_elec_demand.columns = pd.MultiIndex.from_product(
        [df_elec_demand.columns, ["electricity"]]
    )

    # Assign correct total demand to non-res
    non_res_mask = df_buildings["use"] != "Residential"
    df_buildings.loc[non_res_mask, "demand_tot_list"] = df_buildings.loc[
        non_res_mask, "bus"
    ].map(lambda bus: df_elec_demand[bus].sum().tolist())
...
def create_pro_elec(consumer_bus_list):
    ...
    df_pro_base[
        ...
        config.IMP_INST_CAP,
        config.IMP_CAP_UP,
        config.IMP_INV_COST_FIX,
        config.IMP_INV_COST,
        config.IMP_FIX_COST,
        config.IMP_VAR_COST,
        config.IMP_WACC,
        config.IMP_DEPRECIATION,
        config.IMP_PF_MIN,
    )
...
def create_sto_elec(consumer_bus_list):
    ...
    df_sto[
        ...
        "battery_private",
        "electricity",
        config.BS_INST_CAP_C,
        config.BS_CAP_UP_C,
        config.BS_INST_CAP_P,
        config.BS_CAP_UP_P,
        config.BS_EFF_IN,
        config.BS_EFF_OUT,
        config.BS_DISCHARGE,
        config.BS_EP_RATIO,
        config.BS_INV_COST_P,
        config.BS_INV_COST_C,
        config.BS_FIX_COST_P,
        config.BS_FIX_COST_C,
        config.BS_VAR_COST_P,
        config.BS_WACC,
        config.BS_DEPRECIATION,
    )
