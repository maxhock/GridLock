...
def _get_roof_sections(area, tilt_hist):
    ...
    roof_type = np.random.choice(
        ["flat", "gabled"], p=[config.PROB_FLAT_ROOF, config.PROB_GABLED_ROOF]
    )
    ...
    ### Else assume two opposing gabled roof sections
        ...
        # Sample tilt
        u = np.random.uniform()
        bin_idx = tilt_hist["cum_prob"].searchsorted(
            u
        )  # Find the bin corresponding to the cumulative probability
        row = tilt_hist.iloc[bin_idx]
        tilt = int(
            round(np.random.uniform(row["bin_left"], row["bin_right"]), 0)
        )  # Sample uniformly within the selected bin

        A_eff = round(
            (area / 2) * config.SLANTED_ROOF_UTILIZATION / np.cos(np.radians(tilt)), 3
        )
        cap = round(A_eff * config.PV_AREA_FACTOR, 3)
...
def _calculate_pv_power(lat, lon, altitude, tilt, azimuth, weather_df):
    ...
    site = location.Location(
        latitude=lat, longitude=lon, altitude=altitude, tz="Etc/GMT-1"
    )

    # Define all system parameters
    albedo = (
        config.ALBEDO
    )  # Default value of 0.2 for taking ground reflectance into account
    module_parameters = (
        config.MODULE_PARAMETERS
    )  # define module like average German PV module
    ...
    temperature_model_params = TEMPERATURE_MODEL_PARAMETERS["sapm"][
        "open_rack_glass_glass"
    ]  # Typical for pv roof installations
...
def sample_statistics(df_buildings):
    ...
    df_buildings["roofs"] = df_buildings["area"].apply(
        lambda x: _get_roof_sections(x, config.ROOF_TILT_DIST)
    )
    return df_buildings
...
def create_supim_solar(df_buildings, df_weather, location, altitude):
    ...
    for index, row in df_buildings.iterrows():
        for roof_sec in row["roofs"]:
            ...
            if ts_name in ts_dict.keys():
                pass
            else:
                ts_dict[f"solar_{tilt}_{azim}"] = _calculate_pv_power(
                    location["lat"], location["lon"], altitude, tilt, azim, df_weather
                )
...
def create_pro_solar(df_buildings):
    ...
    df_pro[
        ...
        config.PV_INST_CAP,
        config.PV_INV_COST_FIX,
        config.PV_INV_COST,
        config.PV_FIX_COST,
        config.PV_VAR_COST,
        config.PV_WACC,
        config.PV_DEPRECIATION,
        config.PV_PF_MIN,
    )
...
def create_pro_com_solar(unique_panel_list):
    ...
    df_pro_com_base["Process"] = df_pro_com_base["Commodity"].str.replace(
        "^solar", "Rooftop PV", regex=True
    )
    # Solar input
    ...
    df_pro_com = pd.concat([df_pro_com_in, df_pro_com_out_p], axis=0)[
