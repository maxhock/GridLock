...
def _get_dhw_demand_ghd(df_buildings):
    df_normalized_lps_ghd = (
        pd.read_csv(config.DHW_GHD_PATH, skiprows=1, header=[0]) * 1000
    )

    # Apply function and create a new DataFrame
    data_dict_ghd = {
        row["bus"]: _get_single_dhw_timeseries_ghd(
            row["type"], row["area"], row["floors"], df_normalized_lps_ghd
        )
    ...
    df_dhw_demand.columns = pd.MultiIndex.from_product(
        [df_dhw_demand.columns, ["water_heat"]]
    )
...
def _get_cop(hp_type, heating_type, df_heat_space, df_heat_water, air_temp, soil_temp):
    ...
    if hp_type == "ASHP":
        ...
        hp_cop_func = config.GSHP_COP
    # elif hp_type=="WSHP": hp_cop_func = config.WSHP_COP
    else:
        raise ValueError("Unknown heat pump type!")
    ...
    numerator = (
        cop_space.values * df_heat_space.values
        + cop_water.values * df_heat_water.values
    )
    cop_total = np.divide(
        numerator,
...
def sample_statistics(df_buildings):
    ...
    df_buildings.loc[df_buildings["constructi"].isna(), ["constructi"]] = (
        np.random.choice(
...
def generate_heat_demands(df_buildings, df_elec_demand, weather_data, zip):
    ...
    scenario.rename(
        ...
        columns={
            "index": "id",
            "type": "building",
            "houses_per_building": "nb_flat",
            "occ_list": "nb_occ",
            "constructi": "year",
        },
    )
    scenario["NWG"] = scenario["building"].apply(
        lambda x: 1 if x not in ["SFH", "MFH", "TH", "AB"] else 0
    )
    scenario["year"] = scenario["year"].str.extract(r"(\d+)(?!.*\d)").astype(int)
    scenario["area"] = scenario["area"] * scenario["floors"]
    scenario["nb_occ"] = scenario["nb_occ"].apply(
        lambda x: [int(round(y, 0)) for y in x]
    )
    scenario["retrofit"] = 0

    # Extract location data
    site_data = pd.read_csv(
        f"{config.DISTGEN_DATA_PATH}/site_data.txt", delimiter="\t", dtype={"Zip": str}
    )
    # print(site_data)
    # Simulate heating
    ...
    df_space_heat.columns = pd.MultiIndex.from_product(
        [df_space_heat.columns, ["space_heat"]]
    )
    df_dhw[df_dhw_ghd.columns] = (
        df_dhw_ghd  # Combine with nonres building results for DHW
    )
...
def generate_hp_cop(df_buildings, df_heat_space, df_heat_water, df_weather):
    ...
    for _, row in df_buildings.iterrows():
        ...
        cop_dict[bus] = _get_cop(
            hp_type, heating_type, space_heat, water_heat, air_temp, soil_temp
        )
    cops_air = pd.concat([cop for _, cop in cop_dict.items()], axis=1)
    cops_air.columns = pd.MultiIndex.from_tuples(
        [(col, "heatpump_air") for col in cop_dict.keys()]
    )
...
def create_pro_heat(consumer_list):
    ...
    df_pro_base[
        ...
        "heatpump_air",
        config.HP_AIR_INST_CAP,
        config.HP_AIR_CAP_UP,
        config.HP_AIR_INV_COST_FIX,
        config.HP_AIR_INV_COST,
        config.HP_AIR_FIX_COST,
        config.HP_AIR_VAR_COST,
        config.HP_AIR_WACC,
        config.HP_AIR_DEPRECIATION,
        config.HP_AIR_PF_MIN,
    )
    # # Heatpump GROUND
    ...
    df_pro_bst[
        ...
        "heatpump_booster",
        config.HP_BST_INST_CAP,
        config.HP_BST_CAP_UP,
        config.HP_BST_INV_COST_FIX,
        config.HP_BST_INV_COST,
        config.HP_BST_FIX_COST,
        config.HP_BST_VAR_COST,
        config.HP_BST_WACC,
        config.HP_BST_DEPRECIATION,
        config.HP_BST_PF_MIN,
    )
    # Heat dummy space
    ...
    df_pro_dmys[
        ...
        config.HDM_INST_CAP,
        config.HDM_CAP_UP,
        config.HDM_INV_COST_FIX,
        config.HDM_INV_COST,
        config.HDM_FIX_COST,
        config.HDM_VAR_COST,
        config.HDM_WACC,
        config.HDM_DEPRECIATION,
        config.HDM_PF_MIN,
    )
    # Heat dummy water
...
def create_sto_heat(consumer_list):
    ...
    df_sto[
        ...
        "heat_storage",
        "common_heat",
        config.TS_INST_CAP_C,
        config.TS_CAP_UP_C,
        config.TS_INST_CAP_P,
        config.TS_CAP_UP_P,
        config.TS_EFF_IN,
        config.TS_EFF_OUT,
        config.TS_DISCHARGE,
        config.TS_EP_RATIO,
        config.TS_INV_COST_P,
        config.TS_INV_COST_C,
        config.TS_FIX_COST_P,
        config.TS_FIX_COST_C,
        config.TS_VAR_COST_P,
        config.TS_WACC,
        config.TS_DEPRECIATION,
    )
