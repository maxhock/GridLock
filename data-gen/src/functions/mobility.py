from config_demand import config
...
def _sample_cars_in_building(occ_list, df_cars_per_household):
    ...
    for n_occ in occ_list:
        ...
        car_list.append(
            np.random.choice(
                subset["vehicle_count"].values, size=1, p=subset["probability"].values
            ).item()
        )
    return car_list, sum(car_list)


def _sample_model_and_commuting(
    n_cars, prob_commuter, df_car_model_dist, df_region, bus
):
    if n_cars == 0:
        return {}
    else:
        ...
        buses = pd.DataFrame(
            [bus] * n_cars, columns=["bus"]
        )  # bus at which car is placed, always the same
        ids = pd.DataFrame(
            range(n_cars), columns=["id"]
        )  # id of car at the bus (starting from 0)
        car_model = pd.DataFrame(
            np.random.choice(
            ...
            columns=["model"],
        )  # sampling a car model based on current sales statistics
        commuter = pd.DataFrame(
            np.random.choice(
            ...
            columns=["schedule"],
        )  # samling whether car is used mainly for commuting
        seeds = pd.DataFrame(
            [
                int(
                    "".join(
...
def _simulate_vehicles(vehicle_configs, weather):
    ...
    # Load BEV specs once
    BEVS = BEVspecs(
        filename=config_folder + "/evspecs.json"
    ...
    for idx, cfg in vehicle_configs.items():
        ...
        with tempfile.TemporaryDirectory() as tmpdb:
            ...
            HI = HeatInsulation(
                True
            )  # Creating the heat insulation by copying the default configuration
            ev_model = BEVS.model(
                ("A", cfg["model"], "0")
            )  # Model instance that contains vehicle parameters
            batteries[idx] = ev_model.parameters["battery_cap"]
            ev_model.parameters["battery_cap"] = (
                100  # Add slack to prevent time consuming infeasibilities (this only affects the sampled charging behavior, but as we only intend to charge at home and assign every other location to home later anyways, this does not matter too much - importantly the real battery cap has to be used later in urbs!!!)
            )
...
def _reallocate_to_home(df_hourly):
    ...
    for ts, row in df_hourly[
        (df_hourly["charging_point"] != "home") & (df_hourly["charge_grid"] > 0)
    ].iterrows():
...
def _consolidate_home_stretches(df_hourly):
    ...
    for stretch_id, stretch_df in home_stretches.groupby("stretch_id"):
        ...
        if not stretch_df.empty:
            ...
            max_charge_allowed = (
                len(all_indices) * config.CAPACITY_HOME_CHARGING - 0.1
            )  # No modification with timestep length here as still in power and not energy frame
...
def sample_statistics(df_buildings, df_region):
    ...
    df_cars_per_hh = config.CARS_PER_HH_BY_REGION[
        config.CARS_PER_HH_BY_REGION["region"] == region
    ]

    # Now sample number of owned cars, and their model + driver type
    df_buildings[["cars_by_flat", "n_cars_tot"]] = df_buildings["occ_list"].apply(
        lambda x: pd.Series(_sample_cars_in_building(x, df_cars_per_hh))
    )
    df_buildings["car_dict"] = df_buildings.apply(
        lambda x: _sample_model_and_commuting(
            x["n_cars_tot"],
            config.PROB_COMMUTING,
            config.CAR_MODEL_DISTRIBUTION,
            df_region,
            x["bus"],
        ),
        axis=1,
...
def get_mobility_demand(vehicles, weather):
    ...
    for key, value in all_timeseries.items():
        df = value.resample("h").agg(
            {  # Resample to hourly, take most occuring driving state if tie take random
                "charge_grid": "sum",
                "charging_point": _most_frequent_random_tie,
            }
        )
        df = _reallocate_to_home(
            df
        )  # Shift all demands occuring outside of home to nearest home
        df = _consolidate_home_stretches(
            df
        )  # Shift all demands with "home"-stretch to end of stretch
        ...
        max_cap_allowed = (
            all_batteries[key] - 0.1
        )  # Have some slack to prevent rounding caused MILP infeasibilities
    ...
    availability = pd.DataFrame(avai_dict).reset_index(drop=True)
    if (
        isinstance(availability.columns, pd.MultiIndex)
        and len(availability.columns.levels) > 1
    ):
        new_ids = [f"charging_station{id}" for id in availability.columns.levels[1]]
        availability.columns = availability.columns.set_levels(new_ids, level=1)
    else:
        ...
        new_ids = []
        availability.columns = pd.MultiIndex.from_arrays(
            [availability.columns, new_ids]
        )

    # Handle mob_demand DataFrame
    mob_demand = pd.DataFrame(demand_dict).reset_index(drop=True)
    if (
        isinstance(mob_demand.columns, pd.MultiIndex)
        and len(mob_demand.columns.levels) > 1
    ):
        new_ids = [f"mobility{id}" for id in mob_demand.columns.levels[1]]
        mob_demand.columns = mob_demand.columns.set_levels(new_ids, level=1)
    else:
...
def create_pro_mob(battery_dict):
    if not battery_dict:
        ...
        df_pro = pd.DataFrame(
            [(bus, f"charging_station{id}") for (bus, id) in battery_dict.keys()],
            columns=["Site", "Process"],
        )
        df_pro[
            [
                "inst-cap",
            ...
            config.CS_INST_CAP,
            config.CS_CAP_UP,
            config.CS_INV_COST_FIX,
            config.CS_INV_COST,
            config.CS_FIX_COST,
            config.CS_VAR_COST,
            config.CS_WACC,
            config.CS_DEPRECIATION,
            config.CS_PF_MIN,
        )
...
def create_com_mob(battery_dict):
    if not battery_dict:
        ...
        df_com = pd.DataFrame(
            [(bus, f"mobility{id}") for (bus, id) in battery_dict.keys()],
            columns=["Site", "Commodity"],
        )
...
def create_pro_com_mob(battery_dict):
    if not battery_dict:
        ...
        df_pro_com_in = pd.DataFrame(
            [
                (f"charging_station{id}", "electricity", "In", 1)
                for id in range(max_id + 1)
            ],
            columns=["Process", "Commodity", "Direction", "ratio"],
        )
        df_pro_com_out = pd.DataFrame(
            [
                (f"charging_station{id}", f"mobility{id}", "Out", 1)
                for id in range(max_id + 1)
            ],
            columns=["Process", "Commodity", "Direction", "ratio"],
        )
...
def create_sto_mob(battery_dict):
    if not battery_dict:
        ...
        df_sto = pd.DataFrame(
            [
                (bus, f"mobility_storage{id}", f"mobility{id}", cap, cap, cap, cap)
                for (bus, id), cap in battery_dict.items()
            ],
            columns=[
                "Site",
                "Storage",
        ...
        df_sto[
            ...
            config.MS_DISCHARGE,
            config.MS_EP_RATIO,
            config.MS_INV_COST_P,
            config.MS_INV_COST_C,
            config.MS_FIX_COST_P,
            config.MS_FIX_COST_C,
            config.MS_VAR_COST_P,
            config.MS_WACC,
            config.MS_DEPRECIATION,
        )
