...
def get_pvgis_tmy_sarah3_dataframe(latitude, longitude):
    ...
    # Make the request
    print(
        f"Requesting TMY data from PVGIS (SARAH3) for coordinates ({latitude}, {longitude})..."
    )
    try:
        ...
        if "time(UTC)" in df.columns:
            ...
            df["time(UTC)"] = df["time(UTC)"].apply(
                lambda x: x.replace(year=config.REF_YEAR)
            )
            ...
            df["time(inst)"] = df["time(UTC+1)"] + pd.DateOffset(
                hours=offset
            )  # actual measurement time in UTC+1
            ...
            if offset >= 0:
                ...
                last_row["time(UTC+1)"] = last_row["time(UTC+1)"] - pd.DateOffset(
                    years=1
                )
...
def get_open_meteo_soil_temperature(lat, lon, selected_months):
    ...
    for id in range(len(selected_months)):
        ...
        if month == 2:
            last_day = 28  # Always end on February 28
        else:
