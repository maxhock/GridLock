...
class Grid:
    def __init__(self, settings):
        ...
        ### Basic grid data
        self.df_buildings, self.df_region, self.df_weather_raw = (
            self.SF.get_input_data()
        )
        self.region = int(
            self.df_region["regio7"]
        )  # regiostar region used for mobility statistics
        self.plz = str(self.df_region["plz"]).zfill(
            5
        )  # plz of assumed grid position (not of pylovo grid used as representation)
        self.location = {
            "lat": self.df_region[
                "lat"
            ],  # lattitude of transformer position used for weather data
            "lon": self.df_region["lon"],
        }  # longitude of transformer position used for weather data
        ...
        self.battery_dict = (
            {}
        )  # Holds mobility battery capacities for every vehicle generated by emobpy {(bus, veh_id): cap, ...}
    ...
    def retrieve_weather(self):
        if self.settings["weather_data_exists"]:
            ...
            self.df_weather_raw, self.altitude, selected_months = (
                wth.get_pvgis_tmy_sarah3_dataframe(
                    self.location["lat"], self.location["lon"]
                )
            )
            # Add dew point temperature necessary for vehicle simulation
            self.df_weather_raw["dew_point"] = wth.get_dew_point(
                self.df_weather_raw["temp_air"],
                self.df_weather_raw["relative_humidity"],
            )
            # Add soil temperature (1.00-2.55m) necessary for ground source heat pumps
            self.df_weather_raw["soil_temp"] = wth.get_open_meteo_soil_temperature(
                self.location["lat"], self.location["lon"], selected_months
            )
    ...
    def generate_solar(self):
        ...
        self.df_supim_solar = slr.create_supim_solar(
            self.df_buildings[["bus", "roofs"]],
            self.df_weather_raw,
            self.location,
            self.altitude,
        )
    ...
    def generate_heat(self):
        ...
        self.df_demand_elec = self._add_output_data_daylight_saving_shift(
            self.df_demand_elec
        )  # Now we can also adjust elec output data

        # Now, obtain heat demands
        if self.settings["parallel"]:
            # Run parallel jobs
            print(
                f"Generating heat demands for {len(self.df_buildings)} building(s) with {sum(self.df_buildings['houses_per_building'])} flat(s)..."
            )
            building_subsets = self.partition_df_by_cpu(
                self.df_buildings, self.settings["n_cpu"], "houses_per_building"
            )
            col_subsets = [
                [(col, "electricity") for col in subset["bus"].values]
                for subset in building_subsets
            ]
            job_args = [
                (
                    subset.reset_index(drop=True),
                    df_elec_input[col_subsets[i]],
                    [np.array(df_wth_input[col]) for col in ["dni", "dhi", "temp_air"]],
                    self.plz,
                )
            ...
            self.df_demand_heat_space = pd.concat(
                [results[i][0] for i in range(len(results))], axis=1
            ).sort_index(axis=1, level=[0])
            self.df_demand_heat_water = pd.concat(
                [results[i][1] for i in range(len(results))], axis=1
            ).sort_index(axis=1, level=[0])
        else:
            self.df_demand_heat_space, self.df_demand_heat_water = (
                heat.generate_heat_demands(
                    self.df_buildings,
                    df_elec_input,
                    [np.array(df_wth_input[col]) for col in ["dni", "dhi", "temp_air"]],
                    self.plz,
    ...
    def generate_mobility(self):
        ...
        print(
            f"Simulating {n_cars} vehicle(s). Expected time {40*cars_per_cpu:.1f} minutes!"
        )
        ...
        if n_cars > 0:
            vehicles = [
                {key: value}
                for build_dict in self.df_buildings["car_dict"].values
            ...
            with ProcessPoolExecutor(max_workers=self.settings["n_cpu"]) as exe:
                # 1) fire off all jobs
                futures = [
                    exe.submit(mbl.get_mobility_demand, v, wth_input) for v in vehicles
                ]
            ...
            ### Concatenate results
            self.df_demand_mobility = pd.concat(
                [results[i][0] for i in range(len(results))], axis=1
            )
            self.df_tve_mobility = pd.concat(
                [results[i][1] for i in range(len(results))], axis=1
            )
        ...
        self.df_demand_mobility = self._add_output_data_daylight_saving_shift(
            self.df_demand_mobility, mobility_dmd=True
        )
        self.df_tve_mobility = self._add_output_data_daylight_saving_shift(
            self.df_tve_mobility
        )
    ...
    def create_weather_urbs(self):
        ...
        df_weather_urbs.rename(
            columns={"temp_air": "Tamb", "ghi": "Irradiation"}, inplace=True
        )
        df_weather_urbs.index.name = "t"
        df_weather_urbs.columns = pd.MultiIndex.from_product(
            [["ambient"], df_weather_urbs.columns]
        )
    ...
    def create_demand(self):
        ...
        self.df_demand = pd.concat(
            [
                self.df_demand_elec,
                self.df_demand_heat_space,
                self.df_demand_heat_water,
                self.df_demand_mobility,
            ],
            axis=1,
    ...
    def create_tve(self):
        self.df_tve = pd.concat(
            [self.df_tve_hpcop, self.df_tve_mobility], axis=1
        ).reset_index(drop=True)
        self.df_tve.index.name = "t"

    def create_bsp(self):
        df_bsp = pd.DataFrame(
            [[config.BSP_IMPORT, config.BSP_FEED_IN]] * len(self.df_supim_solar.index),
    ...
    def create_process_commodity(self):
        ...
        df_pro_com_sol = slr.create_pro_com_solar(
            list(self.df_supim_solar.columns.get_level_values(1).unique())
        )

        df_pro_com = pd.concat(
            [df_pro_com_elec, df_pro_com_heat, df_pro_com_mob, df_pro_com_sol], axis=0
        )
    ...
    def _add_input_data_daylight_saving_shift(df_ts):
        ...
        if len(df_ts) == 0:
            ...
            df_ts = pd.concat(
                [df_ts.iloc[:ts_hour1], new_row_df, df_ts.iloc[ts_hour1:]]
            ).reset_index(drop=True)
    ...
    @staticmethod
    def partition_df_by_cpu(
        df: pd.DataFrame, n_cpus: int, count_column: str
    ) -> list[pd.DataFrame]:
        """
        ...
        building_list = list(
            df[count_column].items()
        )  # [(idx_0, cars_0), (idx_1, cars_1), ...]
