...
class Config:
    ...
    DATA_GRID_DIR = (
        "data/grids"  # Directory from which to read the pylovo grid input data
    )
    DATA_STAT_DIR = (
        "data/statistics"  # Directory from which to read data for computing demands
    )
    ...
    ROOF_TILT_DIST = pd.read_csv(
        f"{DATA_STAT_DIR}/general/tilt_distribution.csv", skiprows=1
    )
    ...
    MODULE_PARAMETERS = {
        ...
        "gamma_pdc": -0.0035,
    }  # Temperature coefficient for mono-SI, https://publica.fraunhofer.de/entities/publication/f6b3dc37-454c-4d29-9040-76ce1b8454da
    ...
    INVERTER_PARAMETERS = {
        "pdc0": 1000
        / 1.1,  # Default dc to ac ratio of 1.1: https://pvwatts.nrel.gov/downloads/pvwattsv5.pdf
        "eta_inv_nom": 0.96,  # Nominal inverter efficiency, default value: https://pvwatts.nrel.gov/downloads/pvwattsv5.pdf
        "eta_inv_ref": 0.9637,
    }  # Reference inverter efficiency, default value: https://pvwatts.nrel.gov/downloads/pvwattsv5.pdf

    # --------------------------------------------------------------#
    # ---------------- Electrical Demand Assignment ----------------#
    # --------------------------------------------------------------#
    ### Private households
    ELEC_BY_HHSIZE_CDFS_NOHEAT = pd.read_csv(
        f"{DATA_STAT_DIR}/inhabited_buildings/elec_by_hhsize_cdfs_noheat.csv",
        header=[0, 1],
    )
    HH_SIZE_DISTRIBUTION = pd.read_csv(
        f"{DATA_STAT_DIR}/inhabited_buildings/hh_size_distribution.csv",
        header=[0],
        skiprows=1,
    )
    ...
    TYPE_GHD_DISTRIBUTION = pd.read_csv(
        f"{DATA_STAT_DIR}/uninhabited_buildings/nonresbuilding_usetype_distribution.csv",
        header=[0],
        skiprows=1,
    )
    AGE_GHD_DISTRIBUTION = pd.read_csv(
        f"{DATA_STAT_DIR}/uninhabited_buildings/nonresbuilding_age_distribution.csv",
        header=[0],
        skiprows=1,
    )
    ...
    HOLIDAYS = [
        ...
        360,
        365,
    ]  # Holidays of 2009 in Bavaria, this is chosen to match the original electricity data source
    INITIAL_DAY = [4]  # Initial day of the 1: Monday, ..., 7: Sunday
    DATA_LENGTH = 31536000  # Temporal length of input data in seconds

    # Building design
    T_SET_MIN = (
        20.0  # °C, Required minimum indoor temperature (for heating load calculation)
    )
    T_SET_MIN_NIGHT = 18.0  # °C, Required minimum indoor temperature at night (for heating load calculation)
    T_SET_MAX = (
        23.0  # °C, Required maximum indoor temperature (for cooling load calculation)
    )
    ...
    DHWLOAD = [
        ...
        4662.1,
        3999.8,
    ]  # Watt, Maximal power for domestic hot water for each of the four building types (SFH, MFH, TH and AB)
    MEAN_DRAWOFF_VOL = [
        40,
        40,
        40,
        40,
    ]  # Liters, Mean drawoff DHW volume per day for each of the four building types (SFH, MFH, TH and AB). Source: 12831-3/A100 Table NA.4
    ...
    CARS_PER_HH_BY_REGION = pd.read_csv(
        f"{DATA_STAT_DIR}/general/cars_per_household_by_region.csv",
        dtype={
            "region": int,
            "hh_size": int,
            "vehicle_count": int,
    ...
    CAR_MODEL_DISTRIBUTION = pd.read_csv(
        f"{DATA_STAT_DIR}/general/cars_by_model.csv",
