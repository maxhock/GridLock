...
try:
    ...
    if conf.data_generation.matching_mode not in ["population","distance"]:
        raise ValueError(f"Invalid matching_mode: {conf.data_generation.matching_mode}, must be 'population' or 'distance'")
...
print(f"Succesfully retrieved grid and electrical demand data under name: {SF_paths.split('/')[-1].split('.')[0]}")
