...
def setup_simulator_and_federate(yaml_str: str) -> tuple:
    ...
    cfg = from_dict(data_class=BuildingSimulationConfig, data=yaml_cfg)#, config=Config(cast=[Enum]))
    ...
    pub = h.helicsFederateRegisterGlobalPublication(fed, "timestep_result", h.HELICS_DATA_TYPE_STRING, "")
...
def run_federate(fed, simulator: BuildingSimulator, timestep_result, sub, pub, stop_time):
    ...
    while current_time < stop_time:
        ...
        assert isinstance(timestep_result, SimulationTimestepResult), f"timestep_result must be of type SimulationTimestepResult, got {type(timestep_result)}"
...
if __name__ == "__main__":
    ...
    try:
        with open("house/config.yaml", "r") as f:
            yaml_str = f.read()
        fed, simulator, timestep_result, sub, pub = setup_simulator_and_federate(yaml_str)
        run_federate(
            fed=fed,
            simulator=simulator,
