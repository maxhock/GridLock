...
def create_helics_pubs_from_gym_space(
    ...
    if isinstance(space, gym.spaces.Dict):
        for key, subspace in space.spaces.items():
            pubs.update(
                create_helics_pubs_from_gym_space(
                    fed, subspace, prefix=f"{prefix}{key}."
                )
            )
        ...
        pubs[clean_name] = h.helicsFederateRegisterGlobalPublication(
            fed, clean_name, helics_type, ""
        )
    else:
...
def create_helics_subs_from_gym_space(
    ...
    if isinstance(space, gym.spaces.Dict):
        for key, subspace in space.spaces.items():
            subs.update(
                create_helics_subs_from_gym_space(
                    fed, subspace, prefix=f"{prefix}{key}."
                )
            )
    elif isinstance(space, (gym.spaces.Box, gym.spaces.Discrete)):
...
def setup_environment_and_federate() -> tuple:
    ...
    env_config = from_dict(
        data_class=EnvironmentConfig, data=yaml_cfg, config=Config(cast=[Enum])
    )
    ...
    action_subs = create_helics_subs_from_gym_space(
        fed, environment.action_space, prefix="action."
    )
    ...
    obs_pubs = create_helics_pubs_from_gym_space(
        fed, environment.observation_space, prefix="obs."
    )
    logging.info(f"Observation publications created: {list(obs_pubs.keys())}")

    reward_pub = h.helicsFederateRegisterGlobalPublication(
        fed, "reward", h.HELICS_DATA_TYPE_DOUBLE, ""
    )
    done_pub = h.helicsFederateRegisterGlobalPublication(
        fed, "done", h.HELICS_DATA_TYPE_BOOLEAN, ""
    )
...
def publish_observation(obs_pubs: Dict, observation: Dict[str, Any]):
    for key, pub in obs_pubs.items():
        try:
            ...
            logging.debug(
                f"Key path '{key}' not found in observation dict. Skipping publication."
            )
...
def run_federate(
    fed, env, initial_obs, action_subs, obs_pubs, reward_pub, done_pub, stop_time
):
