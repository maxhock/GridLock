...
def main() -> None:
    ...
    config_path = Path(
        args.config
        if args.config
        else os.environ.get("CONFIG_PATH", "/config/experiment-LV.yml")
    )
    output_dir = Path(
        args.output if args.output else os.environ.get("OUTPUT_DIR", "/config/tmp")
    )
    data_input_dir = Path(
        args.data_input
        if args.data_input
        else os.environ.get("DATA_INPUT_DIR", "/data/input")
    )
